...
const PageDebugger = require("./debug/PageDebugger"); // NEW
const IDMLUtils = require("./utils/IDMLUtils");
const UnitConverter = require("./utils/UnitConverter");
const NextFontMapper = require("./utils/NextFontMapper");
...
class IDMLProcessor {
  constructor(options = {}) {
    ...
    // Initialize page debugger - NEW
    this.pageDebugger = new PageDebugger({
  ...
  async processIDML(filePath) {
    ...
    try {
      ...
      // Log page debug information with enhanced data
      this.pageDebugger.logPageInfo({
        uploadId,
      ...
      // Save debug data
      await this.pageDebugger.saveDebugDataForUpload(uploadId);
  ...
  async associateElementsWithPages() {
    ...
    if (pages.length === 0) {
      ...
      this.pageDebugger.logError("No pages found for element association");
      return;
    }

    console.log(`Found ${pages.length} pages to associate elements with`);
    this.pageDebugger.logPageInfo({
      action: "association_start",
      pageCount: pages.length,
      pageIds: pages.map((page) => page.self),
    });
    ...
    this.elements.forEach((element) => {
      ...
      if (!element.pageId && element.spreadId) {
        ...
        if (pagesInSpread && pagesInSpread.length > 0) {
          ...
          if (pagesInSpread.length === 1) {
            ...
            this.pageDebugger.logElementAssociation({
              elementId: element.self,
              pageId: pagesInSpread[0],
              method: "single_page_in_spread",
              spreadId: element.spreadId,
            });
          }
      ...
      if (!element.pageId && pages.length > 0) {
        ...
        this.pageDebugger.logElementAssociation({
          elementId: element.self,
          pageId: pages[0].self,
          method: "fallback_to_first_page",
          noSpreadId: !element.spreadId,
        });
      }

      if (!assigned) {
        this.pageDebugger.logWarning("Element not assigned to any page", {
          elementId: element.self,
    ...
    // Log the element count per page
    this.pageDebugger.logPageInfo({
      action: "association_complete",
      elementsPerPage,
    });
