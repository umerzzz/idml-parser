...
class DebugAnalyzer {
  ...
  async addComprehensiveTextFormattingDebug(processor) {
    ...
    const debugInfo = {
      ...
      styleDefinitions: processor.styleParser
        ? processor.styleParser.getStyles()
        : {},
    ...
    if (processor.storyParser && processor.storyParser.getStories) {
      ...
      for (const [storyId, story] of Object.entries(stories)) {
        ...
        debugInfo.detailedStoryAnalysis[storyId] = {
          ...
          directFormattingAttributes:
            this.extractDirectFormattingAttributes(story),
    ...
    if (processor.spreads) {
      for (const [spreadId, spread] of Object.entries(processor.spreads)) {
        debugInfo.spreadElementAnalysis[spreadId] = {
          textFrames:
            spread.pageItems?.filter((item) => item.type === "TextFrame") || [],
          textFrameDetails: this.analyzeTextFrameFormatting(
            spread.pageItems || []
          ),
        };
      }
    ...
    debugInfo.xmlStructureAnalysis =
      this.performXMLStructureAnalysis(processor);

    // 5. CREATE FORMATTING ATTRIBUTE MAPPING
    debugInfo.formattingAttributeMapping =
      this.createFormattingAttributeMapping();

    // 6. IDENTIFY MISSING FORMATTING REASONS
    debugInfo.missingFormattingReasons =
      this.identifyMissingFormattingReasons(processor);
  ...
  async addStyleOnlyDebug(processor) {
    ...
    const styleDebug = {
      timestamp: new Date().toISOString(),
      paragraphStyles: processor.styleParser
        ? processor.styleParser.getParagraphStyles &&
          processor.styleParser.getParagraphStyles()
        : {},
      characterStyles: processor.styleParser
        ? processor.styleParser.getCharacterStyles &&
          processor.styleParser.getCharacterStyles()
        : {},
      fontDefinitions: processor.styleParser
        ? processor.styleParser.getFontDefinitions &&
          processor.styleParser.getFontDefinitions()
        : {},
    };
  ...
  async addExtractedTextDebug(processor) {
    ...
    const stories =
      processor.storyParser && processor.storyParser.getStories
        ? processor.storyParser.getStories()
        : {};
    const textDebug = {
      timestamp: new Date().toISOString(),
      stories: Object.fromEntries(
        Object.entries(stories).map(([storyId, story]) => [
          storyId,
          {
            ...
            whitespaceSample: (story.content?.plainText || "")
              .replace(/[^\s]/g, "_")
              .substring(0, 100),
          },
        ])
  ...
  analyzeStoryStructure(story) {
    const structure = {
      ...
      textFormattingKeys: story.textFormatting
        ? Object.keys(story.textFormatting)
        : [],
      ...
      if (typeof obj === "object" && obj !== null) {
        Object.keys(obj).forEach((key) => {
          ...
          if (key === "CharacterStyleRange") {
            ...
            key.startsWith("@_") &&
            IDMLUtils.isFormattingAttribute(key)
          ) {
        ...
        if (obj.CharacterStyleRange) {
          ...
          charRanges.forEach((range) => {
            ranges.push({
              ...
              allAttributes: Object.keys(range).filter((k) =>
                k.startsWith("@_")
              ),
              contentLength: range.Content
                ? Array.isArray(range.Content)
                  ? range.Content.join("").length
                  : String(range.Content).length
                : 0,
              rawRange: range,
            });
          });
          ...
          paraRanges.forEach((range) => {
            ranges.push({
              ...
              characterStyleRangeCount: range.CharacterStyleRange
                ? Array.isArray(range.CharacterStyleRange)
                  ? range.CharacterStyleRange.length
                  : 1
                : 0,
              rawRange: range,
            });
  ...
  extractDirectFormattingAttributes(story) {
    ...
    console.log(
      `Found ${directFormatting.length} direct formatting attributes`
    );
      ...
      if (typeof obj === "object" && obj !== null) {
        Object.keys(obj).forEach((key) => {
          ...
          if (key === "@_AppliedParagraphStyle" && value) {
            ...
            (key === "@_FillColor" || key === "@_StrokeColor") &&
            value
          ) {
  ...
  analyzeStyleHierarchy(styleParser) {
    ...
    return {
      paragraphStyleHierarchy: this.extractStyleHierarchy(
        styleParser.getStyles().paragraph
      ),
      characterStyleHierarchy: this.extractStyleHierarchy(
        styleParser.getStyles().character
      ),
  ...
  findMissingFonts(styleParser) {
    ...
    // Find missing fonts
    const missing = Array.from(usedFonts).filter(
      (font) => !availableFonts.has(font)
    );
  ...
  performXMLStructureAnalysis(processor) {
    ...
    if (processor.spreads) {
      Object.entries(processor.spreads).forEach(([spreadId, spread]) => {
        analysis.spreadXMLStructure[spreadId] =
          IDMLUtils.getXMLStructure(spread);
      });
    }
  ...
  identifyMissingFormattingReasons(processor) {
    ...
    if (processor.styleParser) {
      ...
      if (Object.keys(styles.paragraph).length === 0) {
        reasons.push(
          "No paragraph styles extracted - check Resources/Styles.xml parsing"
        );
      }

      if (Object.keys(styles.character).length === 0) {
        reasons.push(
          "No character styles extracted - check Resources/Styles.xml parsing"
        );
      }
    } else {
    ...
    if (processor.storyParser) {
      ...
      const storiesWithFormatting = Object.values(stories).filter((story) =>
        story.content?.formattedContent?.some(
          (item) => item.formatting && Object.keys(item.formatting).length > 1
        )
      );

      if (storiesWithFormatting.length === 0) {
        reasons.push(
          "No stories have detailed formatting - check CharacterStyleRange extraction"
        );
      }
    }
    ...
    if (processor.elements) {
      const textFrames = processor.elements.filter(
        (el) => el.type === "TextFrame"
      );
      ...
      if (textFrames.length > 0 && linkedFrames.length === 0) {
        reasons.push(
          "Text frames not properly linked to stories - check parentStory references"
        );
      }
    }
