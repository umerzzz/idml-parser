...
class StyleParser {
  ...
  extractParagraphStyles(styleGroup) {
    const extractStylesRecursively = (group) => {
      if (group.ParagraphStyle) {
        const styles = Array.isArray(group.ParagraphStyle) ? group.ParagraphStyle : [group.ParagraphStyle];
      ...
      if (group.ParagraphStyleGroup) {
        const subGroups = Array.isArray(group.ParagraphStyleGroup) ? group.ParagraphStyleGroup : [group.ParagraphStyleGroup];
  ...
  extractFontFromStyle(style) {
    ...
    let fontRef = style['@_AppliedFont'] || 
                 style['@_FontFamily'] || 
                 style['@_Font'] || '';
  ...
  extractCharacterStyles(styleGroup) {
    const extractStylesRecursively = (group) => {
      if (group.CharacterStyle) {
        const styles = Array.isArray(group.CharacterStyle) ? group.CharacterStyle : [group.CharacterStyle];
      ...
      if (group.CharacterStyleGroup) {
        const subGroups = Array.isArray(group.CharacterStyleGroup) ? group.CharacterStyleGroup : [group.CharacterStyleGroup];
        subGroups.forEach(extractStylesRecursively);
      }
    };
  ...
  async extractFonts(fontsData) {
    ...
    if (fonts.FontFamily) {
      const fontFamilies = Array.isArray(fonts.FontFamily) ? fonts.FontFamily : [fonts.FontFamily];
      ...
      fontFamilies.forEach(family => {
        ...
        if (family.Font) {
          const fontList = Array.isArray(family.Font) ? family.Font : [family.Font];
  ...
  async extractGraphics(graphicsData) {
    ...
    // Extract Colors
    if (graphics.Color) {
      const colors = Array.isArray(graphics.Color) ? graphics.Color : [graphics.Color];
    ...
    if (graphics.Gradient) {
      const gradients = Array.isArray(graphics.Gradient) ? graphics.Gradient : [graphics.Gradient];
  ...
  extractGradientStops(gradient) {
    ...
    if (gradient.GradientStop) {
      const stopList = Array.isArray(gradient.GradientStop) ? gradient.GradientStop : [gradient.GradientStop];
  ...
  resolveStyleFormatting(formatting) {
    ...
    const hasAnyFormatting = formatting.paragraphStyle || formatting.characterStyle || 
                           formatting.fontStyle || formatting.fontReference || formatting.fontSize;
  ...
  getStoryStyleSummary(story) {
    ...
    if (story.content?.formattedContent?.length > 0) {
      const firstFormatted = story.content.formattedContent.find(item => 
