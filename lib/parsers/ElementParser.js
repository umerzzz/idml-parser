...
class ElementParser {
  ...
  createElementPositionMapFixed() {
    ...
    this.elements.forEach((element, index) => {
      ...
      // Apply coordinate system transformation using CONVERTED values
      // FIXED: Convert coordinate offset to pixels as well since we're working with pixel coordinates
      const offsetX =
        this.unitConverter &&
        this.unitConverter.isSupportedUnit(this.documentUnits)
      ...
      const webX = convertedBounds.left + convertedTransform.tx + offsetX;
      const webY = convertedBounds.top + convertedTransform.ty + offsetY;

      console.log(
        `   📍 Web coordinates (using converted values): (${webX}, ${webY}) ${convertedBounds.width}x${convertedBounds.height}`
      );

      // Create position object - now using converted values
      element.position = {
