...
class StoryParser {
  ...
  convertFontSizeToPixels(fontSize) {
    ...
    if (this.unitConverter.isSupportedUnit(fontUnits)) {
      ...
      console.log(
        `📐 Converted font size: ${fontSize} ${fontUnits} → ${pixelSize} px`
      );
      return pixelSize;
    }
        ...
        if (element.CharacterStyleRange) {
          ...
          ranges.forEach((range, rangeIndex) => {
            ...
            const formatting = {
              ...
              fontSize: range["@_PointSize"]
                ? this.convertFontSizeToPixels(parseFloat(range["@_PointSize"])) // ADDED: Convert to pixels
                : null,
              originalFontSize: range["@_PointSize"]
                ? parseFloat(range["@_PointSize"])
                : null, // ADDED: Preserve original
            ...
            if (rangeIndex < ranges.length - 1) {
              ...
              if (!shouldSkipSpace) {
                ...
                debugInfo.push({
                  ...
                  nextStyle,
                  reason:
                    "Default space insertion - words likely split across ranges",
