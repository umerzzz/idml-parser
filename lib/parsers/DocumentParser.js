...
class DocumentParser {
    ...
    if (document && document.Page) {
      ...
      docPages.forEach((page) => {
        pages.push({
          ...
          name: page["@_Name"] || "",
          appliedMaster: page["@_AppliedMaster"] || "",
          geometricBounds: this.convertPageBoundsToPixels(
            IDMLUtils.parseGeometricBounds(page["@_GeometricBounds"])
          ),
          itemTransform: IDMLUtils.parseTransform(page["@_ItemTransform"]),
    ...
    if (this.spreads && Object.keys(this.spreads).length > 0) {
      ...
      if (firstSpread.pages && firstSpread.pages.length > 0) {
        ...
        if (firstPage.geometricBounds) {
          ...
          // FIXED: Convert to pixels only once using UnitConverter
  ...
  // NEW: Convert page geometric bounds to pixels
  convertPageBoundsToPixels(bounds) {
    if (!bounds || !this.unitConverter) {
      return bounds;
    }
    ...
    if (this.unitConverter.isSupportedUnit(units)) {
      const convertedBounds = {
        top: this.unitConverter.toPixels(bounds.top, units),
        left: this.unitConverter.toPixels(bounds.left, units),
        bottom: this.unitConverter.toPixels(bounds.bottom, units),
        right: this.unitConverter.toPixels(bounds.right, units),
        width: this.unitConverter.toPixels(bounds.width, units),
        height: this.unitConverter.toPixels(bounds.height, units),
        _convertedToPixels: true,
      };

      console.log(
        `📐 Converted page bounds: ${bounds.width}x${bounds.height} ${units} → ${convertedBounds.width}x${convertedBounds.height} px`
      );
