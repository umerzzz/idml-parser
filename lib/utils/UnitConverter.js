...
class UnitConverter {
  ...
  convertObjectToPixels(values, fromUnit) {
    ...
    // FIXED: Check if already converted
    if (values._convertedToPixels) {
      console.log("📐 Object already converted to pixels, skipping conversion");
      return values;
    }
    ...
    // FIXED: Mark as converted
    converted._convertedToPixels = true;
  ...
  convertDimensions(dimensions) {
    ...
    // FIXED: Check if already converted
    if (dimensions._convertedToPixels) {
      console.log(
        "📐 Dimensions already converted to pixels, skipping conversion"
      );
      return dimensions;
    }
   ...
   */
  isValueInPixels(value, unit) {
    if (!unit) return true;
    return this.CONVERSIONS_TO_INCHES[unit] === null;
  }

  /**
   * Safe conversion that prevents double conversion
   ...
   */
  safeToPixels(value, fromUnit, alreadyConverted = false) {
    if (typeof value !== "number" || isNaN(value)) {
      return 0;
    }
    ...
    return this.toPixels(value, fromUnit);
  }
