 ...
 * @param {number} containerWidth - The container width (optional, for smart justify)
 * @param {number} textLength - The text length (optional, for smart justify)
...
export const getTextAlign = (
  ...
  const alignments = {
    ...
    RightAlign: "right",
    CenterAlign: "center",
    LeftJustified: "left", // IMPROVED: Use left instead of justify for better readability
    RightJustified: "right", // IMPROVED: Use right instead of justify for better readability
    CenterJustified: "center",
    FullyJustified: "justify", // Keep justify only for fully justified
  };

  let textAlign = alignments[alignment] || "left";

  // SMART JUSTIFY: Only use justify for wide containers with sufficient text
  if (textAlign === "justify" && containerWidth && textLength) {
    // Only justify if container is wide enough and text is long enough
    const avgCharsPerLine = containerWidth / 8; // Rough estimate: 8px per character
    const estimatedLines = textLength / avgCharsPerLine;

    // Don't justify if:
    // - Container is too narrow (< 300px)
    // - Text is too short (< 3 lines estimated)
    // - Text would result in very few words per line
    if (containerWidth < 300 || estimatedLines < 3 || avgCharsPerLine < 30) {
      textAlign = "left";
    }
  }
