...
export const ensureTextContrast = (
  ...
  const isLightBackground = () => {
    ...
    try {
      ...
      console.log(`🎨 Background luminance check: ${backgroundColor} -> ${luminance.toFixed(3)}`);
      ...
      console.log(`🎨 Luminance calculation failed for ${backgroundColor}, being conservative`);
      return true;
    }
  };
  ...
  if (isLightBackground()) {
    console.log(
      `🎨 Light background detected (${backgroundColor}), preserving text color: ${textColor}`
    );
    return textColor;
  }
  ...
  if (contrast >= 2.5) {
    console.log(
      `🎨 Good contrast (${contrast.toFixed(
        2
      )}) for ${textColor} on ${backgroundColor}, keeping original color`
    );
...
export const convertColor = (colorRef, documentData, ColorUtils) => {
  ...
  if (
    ...
    if (
      ...
      console.log(`🎨 Converted color: ${colorRef} -> ${result}`);
      return result;
    }
...
export const getDocumentBackgroundColor = (
  ...
  if (backgroundConfig.mode === "white") {
    console.log("🎨 Using forced white background");
    return "white";
  ...
  if (documentData.elements) {
    ...
    if (fullPageRects.length > 0) {
      ...
      console.log(
        "🎨 Using full-page rectangle as background:",
        bgRect.fill,
        "->",
        detectedColor
      );
