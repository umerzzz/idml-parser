...
export const renderPagePreview = (
  ...
  try {
    ...
    // Debug: Check if elements have linked images
    const elementsWithImages = pageElements.filter(
      (el) => el.linkedImage && el.linkedImage.url
    );
    console.log(
      "🎨 Elements with linked images in preview:",
      elementsWithImages.length
    );
    elementsWithImages.forEach((el) => {
      console.log("🎨 Element with image:", {
        id: el.id,
        name: el.name,
      ...
      >
        ...
        >
            ...
            .map((element, index) => {
              ...
              return (
                ...
                >
                  {(() => {
                    let text =
                      documentData.stories[element.parentStory].text ||
                      "No text";
                    ...
                    if (listFormatting) {
                      text = applyListFormatting(text, listFormatting);
                    }
          ...
          {pageElements.map((element) => {
            ...
            // Debug logging for elements with images only
            if (element.linkedImage && element.linkedImage.url) {
              console.log("🎨 Rendering element with image in preview:", {
                id: element.id,
                type: element.type,
              ...
              >
                ...
                {element.linkedImage && element.linkedImage.url && (
                  <img
                    ...
                    onError={(e) => {
                      console.warn(
                        `❌ Thumbnail image failed to load: ${element.linkedImage.url}`
                      );
                    ...
                    onLoad={() => {
                      console.log(
                        `✅ Thumbnail image loaded successfully: ${element.linkedImage.url}`
                      );
                    }}
                  />
                )}
                ...
                {!isTextFrame && !isContentFrame && !element.linkedImage && (
                  <div
                    style={{
                      width: "100%",
                      height: "100%",
                      backgroundColor: "rgba(128, 128, 128, 0.2)", // Default gray
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: "8px",
                      ...
                      fontWeight: "bold",
                      border: "1px solid #999", // Default gray
                      borderRadius: "4px",
                    }}
                  >
                    {element.name === "Bulleted List"
