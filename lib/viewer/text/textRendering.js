  ...
  return story.formattedContent
    .map((content, index) => {
      ...
      if (
        ...
        // Style resolution completed
      }
      if (
        finalFontStyle === "italic" &&
        (!formatting.fontStyle || formatting.fontStyle === "Regular")
      ) {
        // Unexpected italic styling detected
      }
      ...
      const style = {
        ...
        color: (() => {
          const converted = convertColor(formatting.fillColor);
          const base =
            !converted || converted === "transparent" ? "black" : converted;
          return ensureTextContrast(base, backgroundColor);
        })(),
        textAlign: getTextAlign(
        ...
        ...(function () {
          ...
          if (content.text && content.text.length > 0) {
            // No baselineShift found for text
          }
      ...
      if (
        ...
        // Space insertion analysis completed
      }
...
export const getStoryStyles = (
  ...
  if (baselineShift !== null && baselineShift !== undefined) {
    // Story-level vertical alignment applied
  } else {
    // No story-level baselineShift found
  }
