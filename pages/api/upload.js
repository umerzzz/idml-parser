...
const upload = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      ...
      const uploadDir = path.join(process.cwd(), 'uploads', req.uploadTimestamp);
...
function createComprehensiveProcessedData(rawData, moduleData = {}) {
  ...
  const processedData = {
    ...
    document: {
      ...
      dimensions: rawData.pageInfo?.dimensions || { width: 612, height: 792, units: 'Points' },
    ...
    pageInfo: {
      ...
      dimensions: rawData.pageInfo?.dimensions || { width: 612, height: 792, units: 'Points' },
      margins: rawData.pageInfo?.margins || { top: 0, bottom: 0, left: 0, right: 0 }
    },
    ...
    stories: Object.keys(rawData.stories || {}).reduce((acc, key) => {
      ...
      if (story) { // Include ALL stories, not just those with text
        acc[key] = {
    ...
    ...Object.keys(rawData).reduce((acc, key) => {
      if (!['document', 'pageInfo', 'elements', 'stories', 'debug22', 'packageInfo'].includes(key)) {
        acc[key] = rawData[key];
      }
      return acc;
    }, {})
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isPackageUpload) {
      ...
      const imageExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.tiff', '.tif', '.bmp', '.svg', '.eps', '.ai', '.psd'];
      req.files.forEach(file => {
        if (file.filename !== idmlFile.filename && 
            imageExtensions.some(ext => file.filename.toLowerCase().endsWith(ext))) {
          
          const targetPath = path.join(packageStructure.linksFolder, file.filename);
          
          // Move file to Links folder
          try {
            fs.renameSync(file.path, targetPath);
            // Update resource map
            packageStructure.resourceMap.set(path.basename(file.filename), targetPath);
...
const documentParser = new DocumentParser(elementParser);
...
let detailedAnalysis = { suspiciousFiles: [], largeBinaryFiles: [], filesByType: {} };
try {
...
let spreadAnalysis = { imageReferences: [], linkReferences: [], placedContentDetails: [] };
try {
...
if (extractedImages.length === 0 && idmlContents.filter(f => IDMLUtils.isImageFile(f)).length > 0) {
  try {
